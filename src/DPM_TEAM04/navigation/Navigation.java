 ...
 * 
 * @deprecated Use Driver instead as it incorporates the use of Coordinate and
 *             DirectedCoordinate
 * 
...
public class Navigation {
	...
	/**
	 * This method makes the robot navigate to the desired coordinates, within a
	 * range of error.
	 * 
	 * @param x
	 *            Position desired in x
	 * @param y
	 *            Position desired in y
	 */
	public void travelTo(double x, double y) {
		...
		while (true) {
			this.navigating = true; // If the path is now clear, the robot is
									// navigating again
			trueX = odometer.getPosition().getX(); // Get the X and Y values
													// from the odometer
			...
			if ((trueX <= (this.x + Resources.NAVIGATION_POSITION_BANDWIDTH) && trueX >= (this.x - Resources.NAVIGATION_POSITION_BANDWIDTH))
				...
				// If the robot is close enough to the position it needs to be
				// (position with a range of
				// Resources.NAVIGATION_POSITION_BANDWIDTH)
			...
			if (thetaInRange(
					Resources.NAVIGATION_HEADING_BANDWIDTH,
					this.odometer.getPosition().getTheta(
							CoordinateSystem.POLAR_RAD))) {

				// If the robot's heading is going in the right direction
				// (within a Resources.NAVIGATION_HEADING_BANDWIDTH range)
				this.leftMotor.setSpeed(Resources.SPEED_FORWARD); // Reset
																	// speeds to
																	// go
																	// straight
																	// to the
				...
				// If the robot's heading is not facing the desired theta to go
				// to the position,
				// compute its desired theta and turn to this heading
				turnToDesiredTheta(); // Turn to the desired angle
			}
		}
		this.navigating = false; // When the while loop is exited, the robot is
									// not navigating
		return; // return void
	}

	/**
	 * This method calls the travelTo method, but converting the inputs from a
	 * point coordinate on the map to (x,y) coordinates in cm.
	 * 
	 * @param x
	 *            X component of the point coordinate
	 * @param y
	 *            Y component of the point coordinate
	 */
	 ...
	 * 
	 * @param theta
	 *            The angle at which it needs to turn to. Theta is in radians.
	 * @param wait
	 *            Boolean to set if the method waits until the rotation is
	 *            complete before returning. True sets it to wait. False sets it
	 *            to not wait.
	 */
	public void turnTo(double theta, boolean wait) {
		...
		// Compute both turns (left and right) to compare which one will be the
		// smallest
		double thetaLeftTurn = 0.0;
		double thetaRightTurn = 0.0;
		double currentHeading = this.odometer.getPosition().getTheta(
				CoordinateSystem.POLAR_RAD);
		...
		if (thetaLeftTurn <= thetaRightTurn) {
			...
			this.leftMotor.rotate(
					-convertAngle(Resources.LEFT_WHEEL_RADIUS, Resources.TRACK,
							radsToDegrees(thetaLeftTurn)), true);
			if (wait) {
				this.rightMotor.rotate(
						convertAngle(Resources.RIGHT_WHEEL_RADIUS,
								Resources.TRACK, radsToDegrees(thetaLeftTurn)),
						false); // wait until comleted
			} else {
				this.rightMotor.rotate(
						convertAngle(Resources.RIGHT_WHEEL_RADIUS,
								Resources.TRACK, radsToDegrees(thetaLeftTurn)),
						true); // not wait until comleted
			}
			...
			this.leftMotor.rotate(
					convertAngle(Resources.LEFT_WHEEL_RADIUS, Resources.TRACK,
							radsToDegrees(thetaRightTurn)), true);
			if (wait) {
				this.rightMotor
						.rotate(-convertAngle(Resources.RIGHT_WHEEL_RADIUS,
								Resources.TRACK, radsToDegrees(thetaRightTurn)),
								false); // wait until completed
			} else {
				this.rightMotor
						.rotate(-convertAngle(Resources.RIGHT_WHEEL_RADIUS,
								Resources.TRACK, radsToDegrees(thetaRightTurn)),
								true); // not wait until completed
			}
	...
	/**
	 * This method turns to the desired theta, which is the minimal angle to
	 * reach a destination.
	 */
	public void turnToDesiredTheta() {
		this.leftMotor.stop(true); // Stop the motors and wait until complete
		this.rightMotor.stop(false);
		double desiredTheta = getDesiredTheta(); // Get the desired theta AFTER
													// the motors are stopped
		turnTo(desiredTheta, true); // turnTo the desired heading
	}

	/**
	 * Returns whether turnTo is running, or travelTo is running and isn't
	 * pausing until the path is clear.
	 * 
	...
	/**
	 * Returns true if trueTheta is within a range. The range is determined by
	 * the current position in relation to x and y, plus or minus the bandwidth.
	 * 
	 * @param bandWidth
	 *            Range of error in theta accepted.
	 * @param trueTheta
	 *            Inputs the heading that needs to be checked if it is in range.
	...
	public boolean thetaInRange(double bandWidth, double trueTheta) {
		...
		double distance = Math
				.sqrt((Math.pow(x_diff, 2) + Math.pow(y_diff, 2)));
		...
		} else if ((trueTheta >= thetaRangeLow)
				&& ((trueTheta <= thetaRangeHigh))) {
	...
	public double getDesiredTheta() {
		...
		// position,
		// compute its desired theta and turn to this heading
		x_diff = this.x - odometer.getPosition().getX(); // Compute the X and Y
															// difference
															// (desired and
															// actual values)
		...
		desiredTheta = Math.atan2(x_diff, y_diff); // Get the angle from delta X
													// and Y
		if (desiredTheta < 0) {
			desiredTheta += (2.0 * Math.PI); // If the angle is negative, add
												// 2*PI
		}
	 ...
	 * 
	 * @param clockwise
	 *            Turns clockwise if true. Turns counterclockwise if false.
	 */
	public void turnAround(boolean clockwise) {
		...
		if (clockwise) {
			...
			this.leftMotor.rotate(
					convertAngle(Resources.LEFT_WHEEL_RADIUS, Resources.TRACK,
							radsToDegrees(2 * Math.PI)), true);
			this.rightMotor.rotate(
					-convertAngle(Resources.RIGHT_WHEEL_RADIUS,
							Resources.TRACK, radsToDegrees(2 * Math.PI)), true);
		} else {
	 ...
	 * 
	 * @param acceleration
	 *            Set acceleration of the motors to this input value. Default
	 *            acceleration if the input is -1.
	 */
	...
	/**
	 * Method to turn around at a custom speed (forever) until it is called to
	 * stop.
	 * 
	 * @param clockwise
	 *            Turns clockwise if true. Turns counterclockwise if false.
	 * @param start
	 *            Starts turning if true, stops turning if false.
	 * @param speed
	 *            Desired turning speed.
	 */
	 ...
	 * 
	 * @param distance
	 *            Distance to go forward in cm. Negative distance will go
	 *            backwards.
	 */
	public void goForward(double distance) {
		...
		this.leftMotor.rotate(
				convertDistance(Resources.LEFT_WHEEL_RADIUS, distance), true);
		this.rightMotor.rotate(
				convertDistance(Resources.RIGHT_WHEEL_RADIUS, distance), false);
	 ...
	 * 
	 * @param start
	 *            Starts going backward if true, stops if false.
	 */
	 ...
	 *            Radius of the wheels in cm.
	 * @param width
	 *            Width (track) of the robot in cm.
	 * @param angle
	 *            Angle to turn in radians.
