...
/**
 * Class to represent 2D points in various coordinate systems. Can also convert between coordinate systems
 * as computationally and spacially efficient as possible.
 * @author KareemHalabi
 *
 */
public class Coordinate {
	 ...
	 * @param c desired coordinate system
	 * @param c1 x coordinate if Cartesian, r if Polar
	 * @param c2 y coordinate if Cartesian, theta if Polar or Compass Heading
	 */
	 ...
	 * @param c desired coordinate system
	 * @param c1 new x coordinate if Cartesian, new r if Polar
	 * @param c2 new y coordinate if Cartesian, new theta if Polar or Compass Heading
	 */
	public synchronized void setCoordinate(CoordinateSystem c, double c1, double c2) {
		switch(c) {
		case CARTESIAN:
			this.x = c1;
			this.y = c2;
	...
	/**
	 * Gets the current angle of a non-origin coordinate in polar coordinate systems.
	 * 
	 * @param unit Desired angular unit
	 * @return Angle in @param unit units.
	 * If POLAR_RAD or CARTESIAN selected: returned value is confined to 0 <= theta < 2pi with 0 rad as x axis
	 * If POLAR_DEG selected: returned value is confined to 0 <= theta < 360 with 0 deg as x axis
	 * If HEADING_DEG selected: returned value is confined to 0 <= theta < 360 with 0 deg as y axis
	 * returns NaN if point is at origin
	 */
	...
	public synchronized double[] getCoordinates(CoordinateSystem coordinateSystem) {
	...
	/**
	 * Sets new angle in desired CoordinateSystem. Does nothing if CARTESIAN is selected
	 * @param newTheta new angle in @param dThetaUnit units
	 * @param dThetaUnit desired unit for @param newTheta
	 */
	@SuppressWarnings("incomplete-switch")
	public synchronized void setTheta(double newTheta, CoordinateSystem dThetaUnit) {
		double r = getR();
		double newThetaRad = newTheta * Math.PI / 180; //in case newTheta is not in rads
	...
	/**
	 * Increments angle in desired CoordinateSystem. Does nothing if CARTESIAN is selected
	 * @param dTheta increment to apply in @param dThetaUnit units
	 * @param dThetaUnit desired unit for @param dtheta
	 */
	@SuppressWarnings("incomplete-switch")
	public synchronized void incrementTheta(double dtheta, CoordinateSystem dThetaUnit) {
		double r = getR();
		double thetaRad = getTheta(CoordinateSystem.POLAR_RAD);
		double dthetaRad = dtheta*Math.PI/180; //in case dtheta is in deg or heading deg, otherwise it won't get used
	 ...
	 * @param c Coordinate system of dimensions 
	 * @param dC1 First dimension to increment
	 * @param dC2 Second dimension to increment
	 */
	public synchronized void incrementCoordinate(CoordinateSystem c, double dC1, double dC2) {
	 ...
	 * @param dest Coordiante to compare to
	...
	/**
	 * Returns shortest angular displacement between currentHeading and @param dest
	 * @param dest Coordiante to compare to
	 * @param currentHeading the current heading in @param currentHeadingUnit. In deg assuming input is 0 <= @param currentHeadingUnit < 360
	 * or in rad 0 <= @param currentHeadingUnit < 2pi
	 * @param currentHeadingUnit CoordinateSystem of @param currentHeading
	 * @param returnedUnit Unit of returned angle
	 * @return shortest angular displacement between this and dest. If the two points are the same, 0 is returned
	 */
	public double angleTo(Coordinate dest, double currentHeading, CoordinateSystem currentHeadingUnit, CoordinateSystem returnedUnit) {
	...
	/**
	 * Returns this Coordinate as a Point object for use in LeJOS
	 * geometry classes
