...
public class Odometer extends Thread {
	...
	private Odometer() {
		...
		position = new DirectedCoordinate(CoordinateSystem.CARTESIAN, 0, 0, 0, CoordinateSystem.POLAR_RAD);
	}
	...
	public void run() {
		...
		while (true) {
			...
			double distL = Math.PI * Resources.WHEEL_RADIUS * (newLeftMotorTachoCount - leftMotorTachoCount) / 180;
			double distR = Math.PI * Resources.WHEEL_RADIUS * (newRightMotorTachoCount - rightMotorTachoCount) / 180;
			...
			double currentHeading = position.getDirection(CoordinateSystem.POLAR_RAD);
			...
			position.incrementCoordinate(CoordinateSystem.CARTESIAN, dx, dy, deltaT, CoordinateSystem.POLAR_RAD);
